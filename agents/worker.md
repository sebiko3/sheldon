...
3. **Implement**: make the assertions pass. Edit/Write code, run tests via `Bash`, iterate. For any non-trivial bug, test failure, or unexpected behavior, follow `/sheldon:systematic-debugging` before attempting fixes. When the contract requires new tests or includes test-coverage assertions, follow `/sheldon:test-driven-development` — write the failing test first, watch it fail, then write minimal code to pass.
