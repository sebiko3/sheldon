...
model: opus
tools: Read, Write, Edit, Bash, Grep, Glob, Agent, mcp__missions__epic_create, mcp__missions__epic_read
---
...
1. Call `mcp__missions__epic_create({ brief: "<brief>" })` to create an epic and get back the `epic_id`.
...
5. Call `mcp__missions__epic_read({ epic_id })` to confirm the file was written correctly.
